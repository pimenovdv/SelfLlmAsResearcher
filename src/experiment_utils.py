...
def compute_parameter_pearsons_mode_skewness(model: torch.nn.Module) -> float:
    """
...
def compute_gradient_pearsons_mode_skewness(model: torch.nn.Module) -> float:
    """
...
def compute_activation_pearsons_mode_skewness(model: torch.nn.Module, input_data: torch.Tensor, layer_names: list[str]) -> dict[str, float]:
    """
    ...
    def get_hook(name):
        def hook(model, input, output):
            ...
            activations[name] = float((mean - mode) / std)
        return hook
...
def compute_parameter_pearsons_median_skewness(model: torch.nn.Module) -> float:
    ...
    mean = torch.mean(vec)
    median = torch.median(vec)
    return float(3.0 * (mean - median) / std)

def compute_gradient_pearsons_median_skewness(model: torch.nn.Module) -> float:
    """
...
def compute_activation_pearsons_median_skewness(model: torch.nn.Module, input_data: torch.Tensor, layer_names: list[str]) -> dict[str, float]:
    """
        ...
        def hook(model, input, output):
            ...
            if vec.numel() > 0:
                ...
                if std > 0.0:
                    mean = torch.mean(vec)
                    median = torch.median(vec)
                    activations[name] = float(3.0 * (mean - median) / std)
                else:
                    activations[name] = 0.0
            else:
                activations[name] = 0.0
...
def compute_gradient_interquartile_range(model: torch.nn.Module) -> float:
    """
...
def compute_activation_interquartile_range(model: torch.nn.Module, input_data: torch.Tensor, layer_names: list[str]) -> dict[str, float]:
    """
    ...
    def get_hook(name):
        def hook(model, input, output):
            ...
            if vec.numel() == 0:
                ...
                try:
                    quantiles = torch.quantile(vec, q).tolist()
                    activations[name] = float(quantiles[1] - quantiles[0])
                except RuntimeError:
                    activations[name] = 0.0
        return hook
...
def compute_parameter_bowley_skewness(model: torch.nn.Module) -> float:
    """
    ...
    if vec.numel() == 0:
        return 0.0
    q = torch.tensor([0.25, 0.50, 0.75], dtype=vec.dtype, device=vec.device)
    try:
        quantiles = torch.quantile(vec, q).tolist()
        q1, q2, q3 = quantiles
        denominator = q3 - q1
        if denominator == 0:
            return 0.0
        return float((q3 + q1 - 2.0 * q2) / denominator)
    except RuntimeError:
        return 0.0

def compute_gradient_bowley_skewness(model: torch.nn.Module) -> float:
    """
...
def compute_activation_bowley_skewness(model: torch.nn.Module, input_data: torch.Tensor, layer_names: list[str]) -> dict[str, float]:
    """
            ...
            if vec.numel() > 0:
                ...
                try:
                    ...
                    if denominator == 0:
                        ...
                        activations[name] = float((q3 + q1 - 2.0 * q2) / denominator)
                except RuntimeError:
                    activations[name] = 0.0
            else:
                activations[name] = 0.0
...
def compute_parameter_bimodality_coefficient(model: torch.nn.Module) -> float:
    """
...
def compute_gradient_bimodality_coefficient(model: torch.nn.Module) -> float:
    """
...
def compute_activation_bimodality_coefficient(model: torch.nn.Module, input_data: torch.Tensor, layer_names: list[str]) -> dict[str, float]:
    """
    ...
    def get_hook(name):
        def hook(model, input, output):
            ...
            if K == 0.0:
                ...
                activations[name] = float((S ** 2 + 1.0) / K)
        return hook
...
def compute_parameter_tsallis_entropy(model: torch.nn.Module, alpha: float = 2.0, bins: int = 256) -> float:
    """
    Вычисляет энтропию Тсаллиса для параметров модели.
    """
    import torch
    if alpha == 1.0:
        raise ValueError("alpha cannot be 1.0 for Tsallis entropy.")
    params = [p.data.flatten() for p in model.parameters() if p.numel() > 0]
    if not params:
        return 0.0
    vec = torch.cat(params)
    ...
    p = hist / hist.sum()
    p = p[p > 0]
    tsallis = (1.0 - torch.sum(p ** alpha)) / (alpha - 1.0)
    return float(tsallis.item())

def compute_gradient_tsallis_entropy(model: torch.nn.Module, alpha: float = 2.0, bins: int = 256) -> float:
    """
...
def compute_activation_tsallis_entropy(model: torch.nn.Module, input_data: torch.Tensor, layer_names: list[str], alpha: float = 2.0, bins: int = 256) -> dict[str, float]:
    """
    ...
    def hook(name):
        def fn(module, inp, out):
            ...
            p = p[p > 0]
            tsallis = (1.0 - torch.sum(p ** alpha)) / (alpha - 1.0)
            tsallis_dict[name] = float(tsallis.item())
        return fn
...
def compute_hellinger_distance_between_models(model1: torch.nn.Module, model2: torch.nn.Module) -> float:
    """
    ...
    return float((1.0 / (2.0 ** 0.5)) * torch.sqrt(torch.sum((torch.sqrt(p) - torch.sqrt(q)) ** 2)).item())

def compute_chebyshev_distance_between_models(model1: torch.nn.Module, model2: torch.nn.Module) -> float:
    """
...
def compute_chi_square_distance_between_models(model1: torch.nn.Module, model2: torch.nn.Module) -> float:
    """
    ...
    numerator = (p - q) ** 2
    denominator = p + q + 1e-8

    return float((0.5 * torch.sum(numerator / denominator)).item())
...
def compute_jaccard_similarity_between_models(model1: torch.nn.Module, model2: torch.nn.Module) -> float:
    ...
    intersection = torch.min(p, q).sum()
...
def compute_angular_distance_between_models(model1: torch.nn.Module, model2: torch.nn.Module) -> float:
    """
    Вычисляет угловое расстояние (Angular Distance) между весами двух моделей.
    """
    import math
    cosine_sim = compute_cosine_similarity_between_models(model1, model2)
    cosine_sim = max(-1.0, min(1.0, cosine_sim))
    return float(math.acos(cosine_sim) / math.pi)

def compute_l0_distance_between_models(model1: torch.nn.Module, model2: torch.nn.Module) -> float:
    """
...
def compute_hedges_g_between_models(model1: torch.nn.Module, model2: torch.nn.Module) -> float:
...
def compute_mean_percentage_error_between_models(model1: torch.nn.Module, model2: torch.nn.Module) -> float:
    """
...
def compute_root_mean_squared_percentage_error_between_models(model1: torch.nn.Module, model2: torch.nn.Module) -> float:
    """
...
def compute_root_mean_squared_logarithmic_error_between_models(model1: torch.nn.Module, model2: torch.nn.Module) -> float:
    """
    Computes the Root Mean Squared Logarithmic Error (RMSLE) between the parameters of two models.
    """
    import torch
    params1 = [p.view(-1) for p in model1.parameters() if p.requires_grad]
    params2 = [p.view(-1) for p in model2.parameters() if p.requires_grad]
    ...
    msle = torch.mean((torch.log1p(vec1) - torch.log1p(vec2)) ** 2)
    rmsle = torch.sqrt(msle)
    return float(rmsle.item())


def compute_mean_arctangent_absolute_percentage_error_between_models(model1: torch.nn.Module, model2: torch.nn.Module) -> float:
    """
    Computes the Mean Arctangent Absolute Percentage Error (MAAPE) between the parameters of two models.
    """
    import torch
    params1 = [p.view(-1) for p in model1.parameters() if p.requires_grad]
    params2 = [p.view(-1) for p in model2.parameters() if p.requires_grad]
...
def compute_tanimoto_distance_between_models(model1: torch.nn.Module, model2: torch.nn.Module) -> float:
    """
    Computes the Tanimoto distance between the parameters of two models.
    Tanimoto distance is 1 - sum(x_i * y_i) / (sum(x_i^2) + sum(y_i^2) - sum(x_i * y_i)).
