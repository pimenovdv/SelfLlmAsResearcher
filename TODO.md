...
- [ ] Реализовать базовый эксперимент (Baseline) для задачи Factual Recall (извлечение фактологических знаний).
- [ ] Использование Activation Patching для поиска слоев/голов, ответственных за Factual Recall.
- [ ] Точечный патчинг MLP-слоев для выявления путей передачи информации.
