...
def main():
    ...
    for layer_idx in range(num_layers):
        ...
        for head_idx in range(num_heads):
            handle1 = model.transformer.h[layer_idx].attn.c_proj.register_forward_pre_hook(save_head_hook(head_idx))
            with torch.no_grad():
                model(**inputs_clean)
            handle1.remove()

            handle2 = model.transformer.h[layer_idx].attn.c_proj.register_forward_pre_hook(patch_head_hook(head_idx))
            with torch.no_grad():
