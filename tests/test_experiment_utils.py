...
class TestExperimentUtils(unittest.TestCase):
    ...
    def test_compute_parameter_dispersion_index(self):
        from src.experiment_utils import compute_parameter_dispersion_index
        import torch
        class DummyModel(torch.nn.Module):
            def __init__(self):
                super().__init__()
            ...
            def forward(self, x):
                return self.fc(x)
        model = DummyModel()
        res = compute_parameter_dispersion_index(model)
        self.assertIsInstance(res, float)

    def test_compute_gradient_dispersion_index(self):
        from src.experiment_utils import compute_gradient_dispersion_index
        import torch
        class DummyModel(torch.nn.Module):
            def __init__(self):
                super().__init__()
        ...
        out = model(torch.randn(1, 10))
        out.sum().backward()
        res = compute_gradient_dispersion_index(model)
        self.assertIsInstance(res, float)

    def test_compute_activation_dispersion_index(self):
        from src.experiment_utils import compute_activation_dispersion_index
        import torch
        class DummyModel(torch.nn.Module):
            def __init__(self):
                super().__init__()
        ...
        model = DummyModel()
        x = torch.randn(2, 10)
        res = compute_activation_dispersion_index(model, x, ['fc'])
        self.assertIn('fc', res)
        self.assertIsInstance(res['fc'], float)
