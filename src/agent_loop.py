...
5. ДОКУМЕНТАЦИЯ БИБЛИОТЕК: Если тебе не хватает знаний по API библиотеки (например, `transformer_lens`), ты можешь воспользоваться встроенными MCP инструментами.
   Добавь в свой скрипт:
   ```python
   from src.mcp_tools import resolve_library_id, get_library_docs
   lib_id = resolve_library_id("transformer_lens")
   print(get_library_docs(lib_id))
   ```
